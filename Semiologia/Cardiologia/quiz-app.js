...
const defaultUnit = "cardiopatiaIsquemica";
...
function readTestConfig() {
  try {
    const raw = sessionStorage.getItem("cardiologiaTestConfig");
    if (!raw) return null;
...
function buildFallbackMatchBoard(q) {
  ...
  const groupOrder = [];
  const slotOrderByGroup = {};

  q.matchItems.forEach(item => {
    const groupKey = item.group || item.syndrome || "grupo";

    if (!groupOrder.includes(groupKey)) {
      groupOrder.push(groupKey);
      slotOrderByGroup[groupKey] = [];
    }

    if (!slotOrderByGroup[groupKey].includes(item.slot)) {
      slotOrderByGroup[groupKey].push(item.slot);
    }
  });

  return groupOrder.map(groupKey => ({
    group: groupKey,
    title: humanizeKey(groupKey),
    rows: slotOrderByGroup[groupKey].map(slot => ({
      slot,
      label: humanizeKey(slot),
      subLabel: ""
    }))
  }));
}
...
function renderMatchBoard(q) {
  ...
  board.forEach(card => {
    ...
    titleEl.textContent = card.title || humanizeKey(card.group || card.syndrome);
    ...
    card.rows.forEach(row => {
      ...
      if (row.subLabel) {
        ...
        labelEl.textContent = row.label || humanizeKey(row.slot);
      }

      const zoneEl = document.createElement("div");
      zoneEl.className = "dropZone";
      zoneEl.dataset.group = card.group || card.syndrome || "";
...
function checkMatchQuestion(q) {
  ...
  getDropZones().forEach(zone => {
    ...
    const expectedIds = q.matchItems
      .filter(item => {
        const groupKey = item.group || item.syndrome || "";
        return groupKey === zone.dataset.group && item.slot === zone.dataset.slot;
      })
      .map(item => item.id)
      .sort();
...
function hasQuestionHint(q) {
  return Boolean(q?.hint);
}
...
function render() {
  ...
  questionHintWrap.textContent = q.hint || "";
  ...
  if (q.type === "matching") {
    ...
    questionHintControls.style.display = isTestMode ? "none" : (hasQuestionHint(q) ? "flex" : "none");
